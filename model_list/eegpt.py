...
class Attention(nn.Module):
    ...
    def forward(self, x, freqs=None):
        ...
        if self.is_causal:
            causal_mask = torch.ones(q.size(-2), q.size(-2), dtype=torch.bool, device=q.device).tril()
...
# ── Linear Probe Classifier (BCIC2A / BCIC2B / KaggleERN / PhysioP300) ───────
# Mirrors LitEEGPTCausal in linear_probe_EEGPT_*.py
# Attribute names match checkpoint keys exactly.
class EEGPTLinearProbeClassifier(nn.Module):
    """
    Two-stage linear probe on top of a frozen EEGTransformer.
    Used for: BCIC2A (4-class MI), BCIC2B (2-class MI),
              KaggleERN (2-class), PhysioP300 (2-class).

    chan_conv   : Conv1dWithConstraint  — only when use_chan_conv=True
    chan_scale  : learnable per-channel scale — only when use_chan_scale=True
    target_encoder : EEGTransformer (frozen at inference)
    linear_probe1  : (embed_num*embed_dim → lp1_out)  per time-patch
    linear_probe2  : (n_patches*lp1_out  → num_classes)
    """
    def __init__(self,
                 num_classes,
                 img_size,
        ...
        n_ch = img_size[0]
        if patch_stride is None:
            n_patches = img_size[1] // 64
        else:
            n_patches = (img_size[1] - 64) // patch_stride + 1

        if use_chan_conv:#使用通道卷积
            assert in_channels is not None
            self.chan_conv = Conv1dWithConstraint(in_channels, n_ch, 1, max_norm=1)

        if use_chan_scale:#使用通道缩放
            self.chan_scale = nn.Parameter(
                torch.ones(1, n_ch, 1) + 0.001 * torch.rand(1, n_ch, 1)
            )
        ...
        self.linear_probe1 = LinearWithConstraint(4 * 512, lp1_out, max_norm=1)
        self.linear_probe2 = LinearWithConstraint(n_patches * lp1_out, num_classes, max_norm=0.25)
        self.drop = nn.Dropout(p=dropout)

        self._use_chan_conv  = use_chan_conv
        self._use_chan_scale = use_chan_scale

    def forward(self, x):
        if self._use_chan_conv:
            x = self.chan_conv(x)
        if self._use_chan_scale:
            x = x * self.chan_scale
        ...
        h = self.linear_probe1(self.drop(h))               # (B, N, lp1_out)
        h = h.flatten(1)                                   # (B, N*lp1_out)
        h = self.linear_probe2(h)                          # (B, num_classes)
        return h


# ── Full Fine-tune Classifier (TUEV / TUAB) ───────────────────────────────────
# Mirrors EEGPTClassifier in EEGPT_mcae_finetune_change_tuev.py
# chan_conv here is the heavier Conv2d version with BN+GELU+Dropout.
class EEGPTFullFinetuneClassifier(nn.Module):
    """
    Full fine-tune model: EEGTransformer + EEGTransformerReconstructor + head.
    Used for: TUEV, TUAB (clinical EEG).

    head input dim = n_patches * embed_num * embed_dim
                   = n_patches * 4 * 512
    The original TUEV script hardcodes head input as 30720
    (= 15 patches × 4 × 512).  Pass head_in_features to override.
    """
    def __init__(self,
                 num_classes,
                 img_size,
        ...
        if use_chan_conv:
            ...
            self.chan_conv = nn.Sequential(
                ...
                nn.Conv2d(img_size[0], img_size[0],
                          kernel_size=(1, chan_conv_kernel), groups=img_size[0],
                          **({'padding': 'same'} if chan_conv_kernel != 15 else {})),
                nn.BatchNorm2d(img_size[0]),
                nn.Dropout(0.8),
            )
        ...
        self.head = nn.Sequential(
            ...
            LinearWithConstraint(head_in_features, num_classes),
        )
    ...
    def forward(self, x):
        ...
        x = self.target_encoder(x, self.chans_id.to(x))  # (B, N, embed_num, embed_dim)
        x = x.flatten(1)
        x = self.head(x)
        return x
